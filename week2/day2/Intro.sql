...
-- we can name/alias our result columns with AS and [column name here]
...
SELECT ProductID, Name, StandardCost, ListPrice, ListPrice * 1.08 AS [Price After Tax], ListPrice - StandardCost AS [Profit Margin]
FROM SalesLT.Product;

SELECT Color, Size
FROM SalesLT.Product; -- returns 295 rows

-- filter duplicate rows out with DISTINCT in SELECT clause
...
-- table aliases
-- important when we start selecting from multiple tables
...
-- all customers with first names that start with B
SELECT *
From SalesLT.Customer
...
SELECT SUBSTRING('123456789', 3, 5); -- start index and length of substring, returns 34567
-- string indexes are 1-based in SQL unfortunately
SELECT REPLACE('Hello World', 'World', 'Justin'); -- return Hello Justin
SELECT CHARINDEX(' ','Hello world'); -- returns 6 (first index of that character in the string)
...
-- COUNT is an aggregate function, this means it operates on many values and returns just one value.
