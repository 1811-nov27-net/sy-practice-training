...
-- the red squigglies will complain if you don't put it
-- even though you're only to send that command by itself with highlighting anyway.

-- red squiggly because it is before the create, will still work (if the table exists)
DROP TABLE PS.Pizza;
DROP TABLE PS.Crust;
...
(
	PizzaID INT IDENTITY(1, 1) NOT NULL, -- an identity column starts at 1 and increments by 1
...
-- (either delete or update a reference PK value, or, insert or update an
...
-- this adds an "Active" boolean column to enable "deleting" a row without actually deleting it.
...
ALTER TABLE PS.Crust
	ADD InternalName AS (CONVERT(VARCHAR(20), CrustID) + '_' + Name) PERSISTED;
...
-- you can use function in select statements and where clauses etc.
...
--      if thre is a failure, we must be returned to the original state
...
--  why? because full isolation is lower performance (higher possibility of deadlock).

-- problem				|			solved by isolation level
-- _____________________|___________________________________________			Less Isolation
--						|		read_uncommitted									|
-- dirty read			|		read_committed	(default in SQL Server)				| def: see others uncommitted changes
-- nonrepeatable read	|		repeatable											| def: see others new committed changes
-- phantom read			|		serializable										V def: see others new committed row insertions
--																				More Isolation
